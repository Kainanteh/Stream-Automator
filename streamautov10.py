...
ESCENA_OBJETIVO = "Game"
INTERVALO_CHECK = 1.0
SERVIDOR_BASE = "https://kainanteh.es"
SERVIDOR_SHEETS = "https://kainanteh.es"
USER_CODE_TWITCH = ""
USER_CODE_KICK = ""
TWITCH_USERNAME = ""
KICK_USERNAME = ""
...
LICENSE_KEY = ""
LICENSE_VALID = False
LICENSE_CHECK_URL = f"{SERVIDOR_BASE}/api/validate_license"
...
def validar_licencia_obs():
    ...
    try:
        ...
        response = requests.post(LICENSE_CHECK_URL, json=data)
        ...
        if result.get('valid'):
            ...
            LICENSE_VALID = False
            print(f"❌ Invalid license: {result.get('error', 'Unknown error')}")
            return False
...
def ejecutar_validaciones_pendientes():
    ...
    if campo_actual_editando == "license_key":
        validar_licencia_obs()
    elif campo_actual_editando == "user_code_twitch" and TWITCH_USERNAME:
...
def verificar_licencia_en_funciones():
    def decorador(func):
        def wrapper(*args, **kwargs):
            if not LICENSE_VALID and LICENSE_KEY:
                print("🔒 Function blocked - License not valid")
                return None
            elif not LICENSE_KEY:
                print("🔒 Function blocked - No license configured")
                return None
...
@verificar_licencia_en_funciones()
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=30) as response:
            ...
            if result.get('status') == 'success':
                nuevas_configuraciones = result.get('configuraciones', {})  # ✅ CLAVE CORRECTA
...
def script_description():
    return "Automatic game detection and update system for Twitch and Kick - V9 (With Wait Timer)"
...
def conectar_twitch_boton(props, prop):
    ...
    if not LICENSE_KEY:
        print("❌ First configure and validate your license")
        return True
        
    print("🔗 Opening connection with Twitch...")
    try:
        auth_url = f"{SERVIDOR_BASE}/twitch/auth?license_key={LICENSE_KEY}"
...
def conectar_kick_boton(props, prop):
    ...
    try:
        auth_url = f"{SERVIDOR_SHEETS}/auth/kick?license_key={LICENSE_KEY}"
        webbrowser.open(auth_url)
        print("✅ Browser opened - Follow instructions to get your code")
    except Exception as e:
        print(f"❌ Error opening browser: {e}")
...
def validar_licencia_boton(props, prop):
    print("🔐 Validating license...")
    if validar_licencia_obs():
        print("✅ License validated successfully")
    else:
        print("❌ Invalid license or connection error")
    return True

def on_license_key_modified(props, prop, settings):
    global LICENSE_KEY
    LICENSE_KEY = obs.obs_data_get_string(settings, "license_key")
    iniciar_timer_debounce("license_key")
...
def script_load(settings):
    ...
    global CONFIG_SHEET_URL, ESCENA_OBJETIVO, INTERVALO_CHECK, LICENSE_KEY
    ...
    KICK_USERNAME = obs.obs_data_get_string(settings, "kick_username")
    CONFIG_SHEET_URL = obs.obs_data_get_string(settings, "config_sheet_url")
    ESCENA_OBJETIVO = obs.obs_data_get_string(settings, "escena") or "Game"
    LICENSE_KEY = obs.obs_data_get_string(settings, "license_key") or ""
    
    sistema_activo = True
    
    print(f"🚀 UNIFIED SYSTEM STARTED - Twitch & Kick (V9 with Timer)")
    ...
    if USER_CODE_TWITCH and TWITCH_USERNAME and LICENSE_VALID:
        print("🔄 Verifying Twitch user automatically...")
        enviar_user_id_twitch()
    
    if USER_CODE_KICK and KICK_USERNAME and LICENSE_VALID:
...
def script_properties():
    ...
    obs.obs_properties_add_text(props, "titulo", "🎮 AUTOMATIC TWITCH & KICK SYSTEM V9 (WITH TIMER)", obs.OBS_TEXT_INFO)
    
    obs.obs_properties_add_text(props, "licencia_titulo", "🔐 SYSTEM LICENSE", obs.OBS_TEXT_INFO)
    licencia_prop = obs.obs_properties_add_text(props, "license_key", "🔑 License Key:", obs.OBS_TEXT_DEFAULT)
    obs.obs_property_set_modified_callback(licencia_prop, on_license_key_modified)
    obs.obs_properties_add_button(props, "validar_licencia", "✅ Validate License", validar_licencia_boton)
...
def script_update(settings):
    ...
    global USER_CODE_TWITCH, USER_CODE_KICK, TWITCH_USERNAME, KICK_USERNAME, LICENSE_KEY
...
def script_defaults(settings):
    obs.obs_data_set_string(settings, "escena", "Game")